...
今の公開版は、メモをこのスマホのブラウザ内に保存します。
スプレッドシートへ送る場合は、Google Apps ScriptのWebアプリURLを作ってから有効化します。
...
5. 実行するユーザーは `自分`
6. アクセスできるユーザーは、まず `全員` にする
7. 発行された `/exec` で終わるURLを `app.js` の `GAS_ENDPOINT` に貼る
...
- 送る項目は、担当AI、状態、タスク名、私のメモ、送信日時だけです
