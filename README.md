...
- publicメモも同じ `office_events` に履歴保存され、同じ `office_status` の該当AI社員行に反映します
- フォームに `今どこまで` と `何待ち` の項目を追加すると、public画面に出す進捗がより正確になります
- `app.js` の `GAS_ENDPOINT` にURLを入れると、public画面は `office_status` を読み込んで表示します
