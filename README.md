...
`office_status` は `projectId + agentId + taskKey` で管理します。
同じAI社員が複数プロジェクトや複数タスクに参加しても、別行になるので上書きされません。
...
- フォームに `プロジェクト名`、`タスク名`、`今どこまで`、`何待ち` の項目を追加すると、public画面に出す進捗がより正確になります
- 同じタスクを更新したい場合は、同じ `タスク名` で報告します
